...
def impute_data(X_train, X_val, X_test, features, strategy="median"):
    ...
    if strategy == "median":
        ...
        stats = X_train[features].mode().iloc[0]
    else:
...
if SHOW_PLOTS:
    ...
    lambdas_plot = ridge_lambdas[1:]
    train_plot = train_mses_ridge[1:]
    val_plot = val_mses_ridge[1:]

    plt.plot(lambdas_plot, train_plot, 'o-', label='Train MSE')
    plt.plot(lambdas_plot, val_plot, 'o-', label='Val MSE')

    plt.xscale('log')
    plt.xlabel('λ (Ridge)')
    plt.ylabel('MSE')
    plt.grid(True, ls='--')
    plt.title('Ridge: Train/Val MSE vs λ (λ≥1)')
    plt.axvline(best_lambda_ridge, linestyle='--')
...
def smear_back_transform(y_train_log, train_pred_log, X_test_b, w):
    ...
    y_pred_log = X_test_b @ w
    y_pred_back = np.expm1(y_pred_log) * smear
